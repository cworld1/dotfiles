# https://github.com/matthewtodd/dotfiles/blob/trunk/etc/fish/functions/fish_jj_prompt.fish
    ...
    jj log 2>/dev/null --no-graph --ignore-working-copy --color=always --revisions @ \
      --template 'surround( "(", ")",
        separate( " ", bookmarks.join(", "),
          ...
          coalesce(
            if(empty, 
                coalesce(if(parents.len() > 1, label("empty", "(merged)")),
                label("empty", "(empty)"),
              ),
            ),
